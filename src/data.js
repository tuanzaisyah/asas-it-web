export const note = [
  {
    id: 1,
    topic: "1.1.1 Teknik dalam Pemikiran Komputasional",
    desc: "Menghuraikan tentang proses pemikiran komputasional dan teknik-teknik di dalam pemikiran komputasional",
    video: "../../assets/videos/sample.mp4",
    note: "../../assets/notes/note1/nota1.pdf",
    images: [
      "../../assets/notes/note1/1.png",
      "../../assets/notes/note1/2.png",
      "../../assets/notes/note1/3.png",
      "../../assets/notes/note1/4.png",
      "../../assets/notes/note1/5.png",
      "../../assets/notes/note1/6.png",
    ],
    quiz: [
      {
        question:
          "Pemikiran komputasional ialah satu proses __________ bagi tujuan ________ sesuatu masalah oleh manusia sendiri berbantukan mesin atau kedua-duanya sekali dengan menggunakan konsep asas sains komputer",
        options: [
          "perubahan, menyelesaikan",
          "automasi, mengatasi",
          "inovasi, mengatasi",
          "pemikiran, menyelesaikan",
        ],
        correctOptionIndex: 4,
      },
      {
        question: "Yang manakan BUKAN teknik dalam pemikiran komputasional?",
        options: ["Leraian", "Pengecaman Corak", "Penguraian", "Pengitlakan"],
        correctOptionIndex: 3,
      },
      {
        question: "Yang manakah BUKAN contoh pemikiran komputasional?",
        options: [
          "Meminta kebenaran ibu bapa untuk pergi ke kedai",
          "Merancang perjalanan yang paling pendek untuk ke sekolah",
          "Merancang cara-cara membuat kek",
          "Membahagikan jadual tugas pelajar untuk membersihkan kelas",
        ],
        correctOptionIndex: 1,
      },
      {
        question: "Pilih aturan yang betul bagi proses pemikiran komputasional",
        options: [
          "Masalah dipecahkan, Perkara tidak penting ditinggalkan, Kenal pasti corak yang sama, Sediakan satu model penyelesaian masalah",
          "Masalah dipecahkan, Kenal pasti corak yang sama, Perkara tidak penting ditinggalkan, Sediakan satu model penyelesaian masalah",
          "Masalah dipecahkan, Kenal pasti corak yang sama,  Sediakan satu model penyelesaian masalah, Perkara tidak penting ditinggalkan",
          "Masalah dipecahkan,  Sediakan satu model penyelesaian masalah, Perkara tidak penting ditinggalkan, Kenal pasti corak yang sama,",
        ],
        correctOptionIndex: 2,
      },
      {
        question:
          "Teknik Leraian membolehkan kita memahami sesuatu proses kerja dengan lebih mudah dan membantu kita memberi penerangan yang lebih jelas atau menyediakan algoritma yang lebih tepat",
        options: ["Betul", "Salah"],
        correctOptionIndex: 1,
      },
    ],
  },
  {
    id: 2,
    topic:
      "1.1.2 Penggunaan Teknik Leraian dan Menentukan Langkah Secara Tertib",
    desc: "Teknik Leraina membolehkan kita memahami sesuatu proses kerja dengan lebih mudah dan membantu kita memberi penerangan dengan lebih jelas",
    video: "../../assets/videos/sample.mp4",
    note: "../../assets/notes/note2/nota2.pdf",
    images: [
      "../../assets/notes/note2/1.png",
      "../../assets/notes/note2/2.png",
      "../../assets/notes/note2/3.png",
      "../../assets/notes/note2/4.png",
      "../../assets/notes/note2/5.png",
    ],
    quiz: [
      {
        question:
          "Yang manakah antara berikut merupakan penggunaan teknik leraian dalam kehidupan?",
        options: [
          "Apakah jenis ubat gigi yang digunakan?",
          "Apakah jenis berus gigi yang digunakan?",
          "Berapa kalikah gosokan untuk memberus gigi di sebelah kiri dan kanan masing-masing?",
          "Semua jawapan di atas",
        ],
        correctOptionIndex: 4,
      },
      {
        question: "Teknik leraian melibatkan... ",
        options: [
          "pemecahan suatu masalah atau sistem yang kompleks kepada bahagian-bahagian kecil bagi memudahkan pemahaman dan penyelesaian",
          "Penguraian masalah kepada beberapa bahagian bagi memudahkan menyelesaikan masalah",
          "Pembahagian jenis leraian yang memudahkan pemahaman",
          "Penyatuan beberapa jenis leraian bagi tujuan memudahkan pemahaman",
        ],
        correctOptionIndex: 1,
      },
      {
        question:
          "Andaikan anda ialah seorang juru landskap. Syarikat anda ditugaskan untuk menyiapkan sebuah taman permainan di sebuah kawasan di luar bandar. Anda dipilih untuk mengkaji dan menganggarkan kos yang diperlukan bagi menyiapkan taman permainan.Pernyataan ini merujuk kepada Teknik Leraian.",
        options: ["Betul", "Salah"],
        correctOptionIndex: 1,
      },
      {
        question:
          "Dalam satu soalan Matematik berkaitan pecahan, Rina memecahkan soalan kepada beberapa langkah sebelum menyelesaikannya.",
        options: ["Betul", "Salah"],
        correctOptionIndex: 1,
      },
      {
        question:
          "Ketika membaca buku cerita, Karina hanya membaca bahagian rumusan di akhir sekali untuk mendapatkan isi pentingnya",
        options: ["Betul", "Salah"],
        correctOptionIndex: 2,
      },
    ],
  },
  {
    id: 3,
    topic: "1.1.3 Pengecaman Corak dalam Sesuatu Situasi",
    desc: "Teknik Pengecaman Corak digunakan untuk menyelesaikan masalah melalui pengecaman persamaan corak",
    video: "../../assets/videos/video3.mp4",
    note: "../../assets/notes/note3/nota3.pdf",
    images: [
      "../../assets/notes/note3/1.png",
      "../../assets/notes/note3/2.png",
      "../../assets/notes/note3/3.png",
      "../../assets/notes/note3/4.png",
      "../../assets/notes/note3/5.png",
      "../../assets/notes/note3/6.png",
    ],
    quiz: [
      {
        question: "Apakah yang dimaksudkan dengan pengecaman corak?",
        options: [
          "Menggabungkan masalah yang kompleks daripada pecahan-pecahan kecil",
          "Memecahkan masalah yang kompleks kepada pecahan-pecahan yang lebih kecil",
          "Mencari perbezaan yang terdapat antara masalah dan dalam masalah",
          "Mencari kesamaan yang terdapat antara masalah dan dalam masalah",
        ],
        correctOptionIndex: 4,
      },
      {
        question: "Manakah antara yang berikut merujuk kepada corak?",
        options: [
          "Semua kereta mempunyai roda",
          "Kereta saya warna biru gelap dan berkilat",
          "Kereta saya di import daripada Jepun",
          "Kereta kawan saya mempunyai pemacu MP3",
        ],
        correctOptionIndex: 1,
      },
      {
        question: "Manakah antara yang berikut TIDAK merujuk kepada corak?",
        options: [
          "Semua kucing mempunyai ekor",
          " Kucing saya boleh berenang",
          "Kucing boleh makan baja khas kucing",
          "Kucing kami suka bersiar-siar",
        ],
        correctOptionIndex: 2,
      },
      {
        question:
          "Manakah antara situasi berikut TIDAK merujuk kepada teknik Pengecaman corak?",
        options: [
          "Polis mengenalpasti kumpulan perompak yang melakukan jenayah rompakan bersenjata berdasarkan modus operandi mereka sebelum ini",
          "Guru Sains menyatakan fungsi setiap bahagian dalam satu tumbuhan",
          "Guru meminta anda membeli pen biru. Anda mengabaikan jenama atau jenis pen perlu dibeli",
          "Saya meneruskan perjalanan apabila melihat lampu isyarat hijau, walaupun pada ketika itu saya baru sahaja berada di United Kingdom kerana saya tahu semua negara mengamalkan peraturan yang sama",
        ],
        correctOptionIndex: 3,
      },
      {
        question:
          "Komputer polis boleh mengesan penjenayah dengan menggunakan sistem yang mengenal pasti cap jari",
        options: ["Peniskalaan", "Leraian", "Pengecaman Corak", "Pengitlakan"],
        correctOptionIndex: 3,
      },
    ],
  },
  {
    id: 4,
    topic: "1.1.4 Membuat Keputusan Berdasarkan Aspek Penting",
    desc: "Aspek penting perlu dikenalpasti untuk mengambil kira pilihan yang ada kerana ianya mempengaruhi hasil atau output yang dikehendaki",
    video: "../../assets/videos/sample.mp4",
    note: "../../assets/notes/note4/nota4.pdf",
    images: [
      "../../assets/notes/note4/1.png",
      "../../assets/notes/note4/2.png",
      "../../assets/notes/note4/3.png",
      "../../assets/notes/note4/4.png",
      "../../assets/notes/note4/5.png",
      "../../assets/notes/note4/6.png",
    ],
    quiz: [
      {
        question: "Aspek-aspek penting perlu dikenalpasti____________",
        options: [
          "Selepas membuat keputusan",
          "Semasa membuat keputusan",
          "Sebelum membuat keputusan",
        ],
        correctOptionIndex: 3,
      },
      {
        question:
          "Aspek penting perlu diambil kira sebelum membuat keputusan kerana",
        options: [
          "Mempengaruhi hasil atau output yang dikehendaki",
          "Mudah",
          "Memudahkan",
        ],
        correctOptionIndex: 1,
      },
      {
        question:
          "Komputer akan mencari laluan paling kurang __________ semasa proses menghantar data melalui talian",
        options: ["sempit", "kekangan atau halangan", "tarikan"],
        correctOptionIndex: 2,
      },
      {
        question: "Aspek-aspek penting membolehkan kita___________",
        options: [
          "Membuat pilihan terbaik",
          "Memenuhi syarat",
          "Mudah membuat kerja",
        ],
        correctOptionIndex: 1,
      },
      {
        question:
          "Pemikiran komputasional iaitu kemahiran membuat keputusan berdasrkan aspek penting dapat digunakan dalam kehidupan seharian seperti_____________",
        options: [
          "Bermain game",
          "Penggunaan GPS dalam menentukan laluan terdekat",
          "Membuat kerja rumah",
        ],
        correctOptionIndex: 2,
      },
    ],
  },
  {
    id: 5,
    topic: "1.1.5 Ciri-Ciri Kesamaan dalam Sesuatu Permasalahan",
    desc: "Mengenal pasti kesamaan atau perbezaan dalam sesuatu masalah bagi membantu mereka bentuk algoritma untuk menyelesaikan sesuatu masalah",
    video: "../../assets/videos/sample.mp4",
    note: "../../assets/notes/note5/nota5.pdf",
    images: [
      "../../assets/notes/note5/1.png",
      "../../assets/notes/note5/2.png",
      "../../assets/notes/note5/3.png",
      "../../assets/notes/note5/4.png",
      "../../assets/notes/note5/5.png",
      "../../assets/notes/note5/6.png",
      "../../assets/notes/note5/7.png",
      "../../assets/notes/note5/8.png",
      "../../assets/notes/note5/9.png",
      "../../assets/notes/note5/10.png",
      "../../assets/notes/note5/11.png",
    ],
  },
];

export const team = [
  {
    id: 1,
    img: "../../assets/img/person1.jpeg",
    name: "A'isyah",
  },
  {
    id: 2,
    img: "../../assets/img/person2.jpg",
    name: "Zaimizan",
  },
  {
    id: 3,
    img: "../../assets/img/person3.png",
    name: "Elawatun",
  },
  {
    id: 4,
    img: "../../assets/img/person4.png",
    name: "Leeny",
  },
  {
    id: 5,
    img: "../../assets/img/person5.png",
    name: "Charirn",
  },
];
